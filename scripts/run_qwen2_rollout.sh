...
for rollout_sz in "${rollout_szs[@]}";do
echo "Training with prompt batch size $prompt_bsz"
rm -rf ~/.cache
prompt_bsz=$(( total_bsz / rollout_sz))
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.n=${rollout_sz}\
    ...
    trainer.experiment_name=${model_name}-polaris-GRPO-rollout_sz${rollout_sz}\
